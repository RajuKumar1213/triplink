...
const seedPackages = [
  ...
  },
  {
    ...
    category: "domestic-destinations",
    overview: [
...
export const POST = asyncHandler(async () => {
  ...
  try {
    ...
    // Generate slugs for seed data
    const packagesWithSlugs = seedPackages.map((pkg) => ({
      ...pkg,
      slug: pkg.name
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
    ...
    // Insert seed data
    const packages = await Package.insertMany(packagesWithSlugs);
    ...
    return NextResponse.json(
      { success: false, message: "Error seeding packages" },
      { status: 500 }
    );
  }
});
